...
function Modal({ title, children, onClose, wide }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100 }}>
      <div style={{ background:"white", borderRadius:10, padding:20, width:wide?480:320, maxWidth:"95vw", border:"1px solid #ddd", maxHeight:"90vh", overflowY:"auto", fontFamily:"Arial,sans-serif" }}>
...
// ── PlanViewer (multi-pages + filtre statut) ──────────────────────────────────
    ...
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:6, flexWrap:"wrap" }}>
        ...
        <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)}
          style={{ fontSize:11, padding:"4px 8px", borderRadius:5, border:"1px solid #ddd", background:"white", color:NC.dark, cursor:"pointer" }}>
            ...
            <div style={{ position:"absolute",left:"50%",top:"50%",transform:"translate(calc(-50% + "+tf.x+"px), calc(-50% + "+tf.y+"px)) scale("+tf.s+")",transformOrigin:"center center",pointerEvents:"none" }}>
              ...
              {visibleZT.map((zt,idx)=>(
                <div key={zt.id} onClick={e=>{e.stopPropagation();onZTClick(zt);}}
                  ...
                  <div style={{ fontSize:9,color:STATUSES[zt.status].color,fontWeight:500,padding:"1px 3px",background:"rgba(255,255,255,0.92)",borderRadius:2,display:"inline-block",maxWidth:"100%",overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis" }}>
                    {idx+1}. {zt.label||"—"}
                  </div>
                </div>
              ))}
...
function TodayView({ role, onOpenZone, pushToast }) {
  ...
  const load = useCallback((d) => {
    setLoading(true);
    fetchAllZonesTravailByDate(d).then(data => {
      setItems(data || []);
      setLoading(false);
    }).catch(() => { setLoading(false); pushToast("Erreur chargement", "error"); });
  }, []);
  ...
  const getPath = (zt) => {
    try {
      ...
      const b = n.batiments;
      const c = b.chantiers;
      return c.name + " › " + b.name + " › " + n.name + " › " + z.name;
    } catch { return "—"; }
  };
    ...
    <div style={{ padding:14, fontFamily:"Arial,sans-serif" }}>
      ...
      {items.map(zt => (
        <div key={zt.id} onClick={() => onOpenZone && onOpenZone(zt)}
          style={{ background:"white", border:"1px solid "+(zt.status==="blocked"?"#fcc":"#e8e8e8"), borderLeft:"3px solid "+STATUSES[zt.status].color, borderRadius:8, padding:"10px 14px", marginBottom:8, cursor:"pointer", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8 }}>
          ...
          <span style={{ fontSize:12, color:STATUSES[zt.status].color, background:STATUSES[zt.status].bg, padding:"3px 10px", borderRadius:5, fontWeight:500, whiteSpace:"nowrap" }}>{STATUSES[zt.status].label}</span>
        </div>
      ))}
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetchChantiers().then(d=>{setChantiers(d);setLoading(l=>({...l,chantiers:false}));}).catch(()=>pushToast("Erreur chargement chantiers","error"));
  }, [role]);
  ...
  const handlePlanUpload = async (e) => {
    ...
    try {
      ...
      setCurrentZone(prev=>prev?{...prev,...res}:null);
      setZones(prev=>prev.map(z=>z.id===sel.zone?{...z,...res}:z));
      pushToast("Plan importé avec succès");
  ...
  const saveZT = async () => {
    ...
    try {
      if (editZT.isNew) {
        ...
        await addHistory(zt.id, role, "Création", editZT.label||"");
        pushToast("Zone créée");
      } else {
        const oldStatus = editZT._origStatus;
        ...
        if (oldStatus !== updated.status) await addHistory(updated.id, role, "Statut", oldStatus+" → "+updated.status);
  ...
  const deleteZT = async (id) => {
    ...
    try {
      ...
      pushToast("Zone supprimée");
    } catch(err){pushToast("Erreur: "+err.message,"error");}
    setSaving(false); setEditZT(null);
  };

  const openHistory = async (zt) => {
    setHistoryZT(zt);
    const h=await fetchHistory(zt.id);
    setHistoryData(h||[]);
  };
  ...
  const handleRename = async () => {
    const name=editingName.trim(); if(!name||!editingItem) return;
    setSaving(true);
    try {
    ...
    <div style={{ minHeight:"100vh", background:"#f4f4f4", fontFamily:"Arial,sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      {addingWhat&&(
        <Modal title={"Nouveau "+(addingWhat==="chantier"?"chantier":addingWhat==="batiment"?"bâtiment":addingWhat==="niveau"?"niveau":"zone")} onClose={()=>{setAddingWhat(null);setNewName("");}}>
          ...
          <div style={{ display:"flex",gap:8 }}>
            <SBtn primary onClick={addItem} style={{ flex:1 }} disabled={saving}>{saving?"…":"Créer"}</SBtn>
      ...
      {editingItem&&(
        <Modal title={"Renommer — "+editingItem.item.name} onClose={()=>setEditingItem(null)}>
          ...
          <div style={{ display:"flex",gap:8 }}>
            <SBtn primary onClick={handleRename} style={{ flex:1 }} disabled={saving}>{saving?"…":"Renommer"}</SBtn>
