...
function NavCol({ title, items, selId, onSel, canAdd, onAdd, onEdit, onDelete, badge, loading }) {
  return (
    <div style={{ flex:"1 1 140px", minWidth:120 }}>
      ...
      {items.map((item) => (
        <div key={item.id}
          ...
          <div style={{ display:"flex", gap:3, flexShrink:0 }}>
            ...
            {canAdd && (
              <>
                <button onClick={(e) => { e.stopPropagation(); onEdit(item); }} title="Renommer"
                  style={{ width:22, height:22, borderRadius:4, border:"1px solid #e0e0e0", background:"white", color:NC.gray, cursor:"pointer", fontSize:12, display:"flex", alignItems:"center", justifyContent:"center", padding:0 }}>✎</button>
                <button onClick={(e) => { e.stopPropagation(); onDelete(item); }} title="Supprimer"
                  style={{ width:22, height:22, borderRadius:4, border:"1px solid #fcc", background:"#fff5f5", color:NC.red, cursor:"pointer", fontSize:12, display:"flex", alignItems:"center", justifyContent:"center", padding:0 }}>×</button>
              </>
            )}
          </div>
        </div>
      ...
      {canAdd && (
        <button onClick={onAdd} style={{ fontSize:12, padding:"5px 8px", borderRadius:6, border:"1px dashed "+NC.red, background:"transparent", color:NC.red, cursor:"pointer", width:"100%", fontFamily:"Arial,sans-serif" }}>
          + {title.slice(0,-1)}
        </button>
      )}
    </div>
...
function PlanViewer({ zone, role, onZTClick, onNewZT }) {
  ...
  const s2p = (sx, sy) => {
    ...
    const rw = Math.min(r.width, imgSz.w);
    const rh = rw * imgSz.h / imgSz.w;
  ...
  const onDown = (e) => {
    ...
    const p = gcp(e);
    if (mode === "pan" || !zone || !zone.plan_url) {
      panRef.current = { on:true, sx:p.x, sy:p.y, tx:tf.x, ty:tf.y };
    } else if (mode === "draw" && role === "ca") {
      const pp = s2p(p.x, p.y);
      drwRef.current = { on:true, sx:pp.x, sy:pp.y };
      setDrawRect({ x:pp.x, y:pp.y, w:0, h:0 });
    }
    e.preventDefault();
  };
  ...
  const onMove = (e) => {
    if (e.touches && e.touches.length === 2 && pnchRef.current.on) {
      const nd  = gdist(e.touches);
      const ns  = Math.max(0.15, Math.min(12, pnchRef.current.s * nd / pnchRef.current.d));
      const mid = gmid(e.touches), el = containerRef.current, r = el.getBoundingClientRect();
      const mx  = mid.x - r.left - r.width  / 2;
      const my  = mid.y - r.top  - r.height / 2;
      setTf({
        x: pnchRef.current.tx + mx - pnchRef.current.mx + (pnchRef.current.mx - pnchRef.current.tx) * (1 - ns / pnchRef.current.s),
        y: pnchRef.current.ty + my - pnchRef.current.my + (pnchRef.current.my - pnchRef.current.ty) * (1 - ns / pnchRef.current.s),
        s: ns
      });
    ...
    if (panRef.current.on) {
      setTf(t => ({ ...t, x: panRef.current.tx + p.x - panRef.current.sx, y: panRef.current.ty + p.y - panRef.current.sy }));
    } else if (drwRef.current.on) {
      const pp = s2p(p.x, p.y);
      setDrawRect({ x: Math.min(drwRef.current.sx, pp.x), y: Math.min(drwRef.current.sy, pp.y), w: Math.abs(pp.x - drwRef.current.sx), h: Math.abs(pp.y - drwRef.current.sy) });
    }
    e.preventDefault();
  };

  const onUp = () => {
    pnchRef.current.on = false;
    panRef.current.on  = false;
    if (drwRef.current.on) {
      drwRef.current.on = false;
      if (drawRect && drawRect.w > 0.5 && drawRect.h > 0.5) onNewZT(drawRect);
      setDrawRect(null);
    }
  };
  ...
  return (
    <div style={{ position:"relative", borderRadius:8, overflow:"hidden", border:"0.5px solid #ddd", background:"#1a1a1a", height:"62vh" }}>
      <div style={{ position:"absolute", top:8, right:8, zIndex:20, display:"flex", flexDirection:"column", gap:3 }}>
        {[["−", () => setTf(t => ({ ...t, s: Math.max(0.15, t.s/1.3) }))],
          ["+", () => setTf(t => ({ ...t, s: Math.min(12,   t.s*1.3) }))],
          ["⊡", () => setTf({ x:0, y:0, s:1 })]].map(([l, fn]) => (
        ...
        <div style={{ position:"absolute", top:8, left:8, zIndex:20, display:"flex", gap:4 }}>
          {[["pan","✥ Naviguer"], ["draw","⬜ Dessiner"]].map(([m, l]) => (
            <button key={m} onClick={() => setMode(m)}
              style={{ fontSize:11, padding:"4px 9px", borderRadius:5, border:"1px solid "+(mode===m?NC.red:"#ccc"), background:mode===m?NC.red:"white", color:mode===m?"white":NC.gray, cursor:"pointer", fontWeight:mode===m?700:400 }}>{l}</button>
          ))}
        </div>
      )}
      <div style={{ position:"absolute", bottom:8, right:8, zIndex:20, fontSize:11, color:"#aaa", background:"rgba(0,0,0,0.55)", padding:"2px 6px", borderRadius:4 }}>
        {Math.round(tf.s * 100)}%
      </div>
      <div ref={containerRef}
        style={{ width:"100%", height:"100%", overflow:"hidden", cursor: mode==="draw" && role==="ca" ? "crosshair" : "grab", touchAction:"none" }}
        ...
        {!zone || !zone.plan_url ? (
          ...
          <div style={{ position:"absolute", left:"50%", top:"50%", transform:"translate(calc(-50% + "+tf.x+"px), calc(-50% + "+tf.y+"px)) scale("+tf.s+")", transformOrigin:"center center", pointerEvents:"none" }}>
            ...
            {zone.plan_type === "pdf" && !pdfReady && (
              <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, color:"#aaa" }}>Rendu PDF…</div>
            )}
            ...
            {drawRect && drawRect.w > 0 && (
              <div style={{ position:"absolute", left:drawRect.x+"%", top:drawRect.y+"%", width:drawRect.w+"%", height:drawRect.h+"%", border:"1px dashed "+NC.red, background:NC.redBg, borderRadius:2, pointerEvents:"none", boxSizing:"border-box" }} />
            )}
          </div>
        )}
      </div>
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"#f4f4f4", padding:16, fontFamily:"Arial,sans-serif" }}>
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:36 }}>
        ...
        <div>
          <div style={{ fontSize:18, fontWeight:700, color:NC.dark }}>NEOCLIMA</div>
          <div style={{ fontSize:11, color:NC.gray, letterSpacing:1 }}>FIELD TRACKER</div>
        </div>
      </div>
      {!selRole ? (
        <div style={{ width:"100%", maxWidth:320 }}>
          ...
          {Object.entries(ROLE_LABELS).map(([k, v]) => (
            <button key={k} onClick={() => setSelRole(k)}
              style={{ width:"100%", padding:"13px 18px", borderRadius:8, border:"1px solid #ddd", background:"white", cursor:"pointer", fontSize:14, color:NC.dark, display:"flex", alignItems:"center", gap:12, marginBottom:8, fontFamily:"Arial,sans-serif" }}>
        ...
        <div style={{ width:"100%", maxWidth:290, background:"white", borderRadius:10, border:"1px solid #ddd", padding:24 }}>
          <button onClick={() => { setSelRole(null); setPin(""); setErr(""); }}
            style={{ fontSize:12, color:NC.gray, background:"none", border:"none", cursor:"pointer", padding:0, marginBottom:18 }}>← Retour</button>
          ...
          <input type="password" inputMode="numeric" maxLength={6} value={pin}
            onChange={(e) => setPin(e.target.value)} onKeyDown={(e) => e.key === "Enter" && tryLogin()} autoFocus placeholder="• • • •"
          ...
          <button onClick={tryLogin}
            style={{ width:"100%", padding:10, borderRadius:8, border:"none", background:NC.red, color:"white", cursor:"pointer", fontSize:14, fontWeight:700 }}>Connexion</button>
...
// ── Dashboard ─────────────────────────────────────────────────────────────────
function Dashboard({ allZT, batiments }) {
  const cnt = {};
  ...
  const byDate = {};
  allZT.forEach(zt => {
    const d = zt.date_pose || "Sans date";
    if (!byDate[d]) byDate[d] = {};
    byDate[d][zt.status] = (byDate[d][zt.status] || 0) + 1;
  });
  const dates = Object.keys(byDate).sort();

  const byEq = {};
  allZT.forEach(zt => {
    const eq = zt.equipe || "Non assigné";
    if (!byEq[eq]) byEq[eq] = { total:0, done:0, blocked:0 };
    byEq[eq].total++;
    if (zt.status === "done")    byEq[eq].done++;
    if (zt.status === "blocked") byEq[eq].blocked++;
  });
            ...
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
              <thead>
                <tr style={{ background:"#f8f8f8" }}>
                  ...
                  {Object.entries(STATUSES).map(([k, v]) => (
                    <th key={k} style={{ padding:"5px 6px", color:v.color, fontWeight:500, borderBottom:"1px solid #eee", whiteSpace:"nowrap", fontSize:11 }}>{v.label}</th>
                  ))}
                  ...
                  return (
                    <tr key={d} style={{ background: i%2===0?"#fafafa":"white" }}>
                      ...
                      {Object.keys(STATUSES).map(k => (
                        <td key={k} style={{ padding:"5px 6px", textAlign:"center", color:row[k]?STATUSES[k].color:"#ddd" }}>{row[k] || "—"}</td>
                      ))}
        ...
        {Object.keys(byEq).length === 0 ? <Muted>Aucune équipe.</Muted> : Object.entries(byEq).map(([eq, s]) => (
          <div key={eq} style={{ display:"flex", alignItems:"center", gap:10, padding:"7px 0", borderBottom:"1px solid #f0f0f0", flexWrap:"wrap" }}>
            ...
            <div style={{ flex:1, minWidth:80 }}>
              <div style={{ fontSize:13, color:NC.dark, fontWeight:500 }}>{eq}</div>
              <div style={{ fontSize:11, color:NC.gray }}>{s.total} zone{s.total>1?"s":""}</div>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:6, flex:2, minWidth:120 }}>
              <div style={{ flex:1, height:7, borderRadius:4, background:"#f0f0f0", overflow:"hidden" }}>
                <div style={{ height:"100%", width:pct(s.done,s.total)+"%", background:NC.green }} />
              </div>
...
export default function App() {
  ...
  const handleRename = async () => {
    ...
    } catch(err) { alert("Erreur renommage: " + err.message); }
    setSaving(false);
    setEditingItem(null);
    setEditingName("");
  };
    ...
    <div style={{ minHeight:"100vh", background:"#f4f4f4", fontFamily:"Arial,sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ background:"white", borderBottom:"2px solid "+NC.red, padding:"9px 14px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div>
            <div style={{ fontSize:13, fontWeight:700, color:NC.dark }}>NEOCLIMA</div>
            <div style={{ fontSize:9, color:NC.gray, letterSpacing:1 }}>FIELD TRACKER</div>
          </div>
      ...
      {/* Dashboard */}
      {view === "dashboard" && <Dashboard allZT={allZT} batiments={batiments} />}
      ...
      {view === "nav" && (
        <div style={{ padding:14, flex:1 }}>
          <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
            ...
            {sel.chantier && (
              <NavCol title="Bâtiments" items={batiments} selId={sel.batiment} loading={loading.batiments}
            ...
            {sel.batiment && (
              <NavCol title="Niveaux" items={niveaux} selId={sel.niveau} loading={loading.niveaux}
            ...
            {sel.niveau && (
              <NavCol title="Zones" items={zones} selId={sel.zone} loading={loading.zones}
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10, flexWrap:"wrap" }}>
            ...
            {canEdit && (
              <button onClick={() => fileInputRef.current && fileInputRef.current.click()}
                style={{ fontSize:12, padding:"4px 12px", borderRadius:5, border:"1px solid "+NC.red, background:NC.red, color:"white", cursor:"pointer", marginLeft:"auto", fontWeight:500 }}>
      ...
      {addingWhat && (
        <Modal title={"Nouveau "+(addingWhat==="chantier"?"chantier":addingWhat==="batiment"?"bâtiment":addingWhat==="niveau"?"niveau":"zone")} onClose={() => { setAddingWhat(null); setNewName(""); }}>
          <input autoFocus value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key==="Enter" && addItem()} placeholder="Nom…"
            style={{ width:"100%", marginBottom:12, fontSize:13, boxSizing:"border-box" }} />
      ...
      {editingItem && (
        <Modal title={"Renommer — "+editingItem.item.name} onClose={() => setEditingItem(null)}>
          <Field label="Nouveau nom">
            <input autoFocus value={editingName} onChange={e => setEditingName(e.target.value)} onKeyDown={e => e.key==="Enter" && handleRename()}
      ...
      {editZT && (
        <Modal title={editZT.isNew ? "Nouvelle zone de travail" : "Zone de travail"} onClose={() => setEditZT(null)}>
          {canEdit && (
            <>
              <Field label="Libellé">
                <input value={editZT.label||""} onChange={e => setEditZT(z => ({...z, label:e.target.value}))} placeholder="Ex: Gaine principale RDC"
                  style={{ width:"100%", fontSize:13, boxSizing:"border-box" }} />
              </Field>
              <Field label="Équipe">
                <input value={editZT.equipe||""} onChange={e => setEditZT(z => ({...z, equipe:e.target.value}))} placeholder="Ex: Équipe A"
                  style={{ width:"100%", fontSize:13, boxSizing:"border-box" }} />
              </Field>
            </>
          )}
            ...
            <>
              {(role === "monteur" || role === "chef") && (
                <Field label="Date de pose">
                  <input type="date" value={editZT.date_pose||""} onChange={e => setEditZT(z => ({...z, date_pose:e.target.value}))}
                    style={{ width:"100%", fontSize:13, boxSizing:"border-box" }} />
                </Field>
              )}
              {role === "ca" && (
                <Field label="Date de pose">
                  <div style={{ fontSize:13, color:editZT.date_pose?NC.dark:NC.gray, padding:"6px 0" }}>
                    {editZT.date_pose ? fmtDate(editZT.date_pose) : "Non renseignée"}
                  </div>
                </Field>
              )}
              <Field label="Statut">
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
                  {Object.entries(STATUSES).map(([k, v]) => (
                    <div key={k} onClick={() => setEditZT(z => ({...z, status:k}))}
                      style={{ padding:8, borderRadius:6, border:"1.5px solid "+(editZT.status===k?v.color:"#e0e0e0"), background:editZT.status===k?v.bg:"#fafafa", cursor:"pointer", fontSize:12, color:editZT.status===k?v.color:NC.gray, textAlign:"center", fontWeight:editZT.status===k?700:400 }}>
                      {v.label}
                    </div>
                  ))}
                </div>
              ...
              {editZT.status === "blocked" && (
                <Field label="Raison du blocage">
                  <textarea value={editZT.comment||""} onChange={e => setEditZT(z => ({...z, comment:e.target.value}))} placeholder="Décrivez le problème…" rows={2}
          ...
          {editZT.isNew && (
            <div style={{ fontSize:12, color:NC.gray, padding:"7px 10px", background:"#f8f8f8", borderRadius:6, marginBottom:10, borderLeft:"3px solid "+NC.gray }}>
              Statut et date de pose renseignés par les monteurs.
            </div>
          )}
